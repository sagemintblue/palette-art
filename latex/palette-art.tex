...
\newcommand{\swatch}[6]{
  ...
  % create coordinates
  \coordinate %[label=left:x]
  (x{#4}r{#3}c{#6}) at ($(#1.west)!\rx!(#1.east)$) {};
  \coordinate %[label=left:y]
  (y{#4}r{#3}c{#6}) at ($(#1.north)!\ry!(#1.south)$) {};

  % draw swatch
  \path let
  \p1 = ($\lw*(#1.east)-\lw*(#1.west)$),
  \p2 = ($\lh*(#1.north)-\lh*(#1.south)$)
  in node[
  swatch=#4#5p#6,
  minimum width=\x1,
  minimum height=\y2,
...
\begin{tikzpicture}[
  ...
  % create rectangle "with computation"
  \path let
  ...
  % draw blocks
  \block{page}{7}{Blues}
  %\block[below right=5mm and 0mm of {YlGn}r{6}c0.south west]{RdPu}
  %\block[below right=5mm and 0mm of {RdPu}r{6}c0.south west]{OrRd}
